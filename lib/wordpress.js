...
const DEFAULT_SETTINGS = {
  ...
  contact_phone: '+8801754574545',
  contact_whatsapp: '+8801754574545',
...
function mapBlogPost(post, index, defaults) {
  ...
  return {
    ...
    // Internal single-post route (was the WP permalink / /contact, which broke navigation)
    url: post.slug ? `/blogs/${post.slug}` : fallback?.url || '/blogs',
  }
}
...
export async function getContactPageData() {
  ...
  const phone = settings.contact_phone || '+8801754574545'
  const whatsapp = settings.contact_whatsapp || settings.contact_phone || '+8801754574545'
...
export async function getAllBlogSlugs() {
  const posts = await wpFetch('/wp/v2/posts?per_page=100&_fields=slug')
  return Array.isArray(posts) ? posts.map((p) => p.slug).filter(Boolean) : []
}

export async function getBlogPost(slug) {
  const posts = await wpFetch(
    `/wp/v2/posts?slug=${encodeURIComponent(slug)}&_embed&acf_format=standard`,
  )
